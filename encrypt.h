...
class Encrypt {
    ...
    private:
        ...
        void finalPermutation(std::vector<int>& data);
        void transformRight(std::vector<int>& right, int num);

        
        //methods used by transformRight
        std::vector<int> expandRight(std::vector<int>& right);
        int getSBoxIndex(const std::vector<int>& sixBits);
        std::vector<int> toBits(int value);

        std::vector<int> pPermutation(std::vector<int> keyedRight);

        //method used by multiple functions
        std::vector<int> xorVectors(const std::vector<int>& a, const std::vector<int>& b);


    //     S TABLES USED BY TRANSFORM RIGHT METHOD
    std::vector<int> s1 = {
